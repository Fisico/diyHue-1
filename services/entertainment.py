                ...
                if data[14] == 0: #rgb colorspace
                    while (i < counter):
                        ...
                        if proto in ["native", "native_multi", "native_single"]:
                            ...
                            enableMusic(bridgeConfig["emulator"]["lights"][str(lightId)]["ip"], host_ip)
                            c = YeelightConnections[bridgeConfig["emulator"]["lights"][str(lightId)]["ip"]]
                            print("bri : " + str(bridgeConfig["lights"][str(lightId)]["state"]["bri"]))
                            operation = skipSimilarFrames(lightId, bridgeConfig["lights"][str(lightId)]["state"]["xy"], bridgeConfig["lights"][str(lightId)]["state"]["bri"])
                            if operation == 1:
                                c.command("set_bright", [int(bridgeConfig["lights"][str(lightId)]["state"]["bri"] / 2.55), "smooth", 200])
