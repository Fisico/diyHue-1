    ...
    if v2uuid in v2Resources[element]:
        ...
        for v1Element in ["lights", "groups", "scenes"]:
            for key, obj in bridgeConfig[v1Element].items():
                if obj.id_v2 == v2uuid:
                    v2Resources[element][v2uuid] = obj
        ...
        if resource == "light":
            ...
            if "action" in putDict:
                if putDict["action"] == "start":
                    ...
                    sleep(3)
                    #Popen(["/opt/hue-emulator/entertain-srv", "server_port=2100", "dtls=1", "psk_list=" + authorisation["user"].username + "," + authorisation["user"].client_key])
